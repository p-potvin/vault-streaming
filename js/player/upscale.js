...
function upscaleSetStatus(txt, color) {
    ...
    if (!badge) return;
    // All upscale statuses share an "up" glyph; render it as an SVG icon and
    // escape the (numeric/dynamic) status text before injecting.
    const arrow = (window.icons && window.icons.arrowUp)
        ...
        : '';
    const safe = window.escapeHtml ? window.escapeHtml(String(txt)) : String(txt);
    badge.innerHTML = arrow + safe;
