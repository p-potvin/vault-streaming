...
Write-Host " VAULT EXPLORER AUDIO NORMALIZATION - RECURSIVE FOLDER RUN" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
Write-Host "Translate To: $TranslateTo" -ForegroundColor Gray
Write-Host "Skip Existing:$($SkipExisting.IsPresent)" -ForegroundColor Gray
...
foreach ($File in $Files) {
    ...
    $ArgsList = @("-u", $PythonScript, $File.FullName, $Folder)
    if ($Transcribe) { $ArgsList += "--transcribe" }
    if ($TranslateTo) { $ArgsList += @("--translate-to", $TranslateTo) }
