...
param(
    ...
    [Parameter(Mandatory = $true, Position = 1, HelpMessage = "Effect to apply: denoise or edge")]
    [ValidateSet("denoise", "edge")]
    [string]$Operation,
...
Write-Host " VAULT EXPLORER IMAGEMAGICK ENHANCEMENT - RECURSIVE FOLDER RUN" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
foreach ($File in $Files) {
    ...
    $OpLabel = $Operation
    $OutputPath = Join-Path $ThumbsDir "$($File.BaseName)_$OpLabel$($File.Extension)"
    ...
    $ArgsList = if ($Operation -eq "denoise") {
        @($File.FullName, "-median", "3", $OutputPath)
    }
    else {
        @($File.FullName, "-edge", "1", $OutputPath)
    }
