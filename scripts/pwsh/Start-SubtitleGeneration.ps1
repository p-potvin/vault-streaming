...
Write-Host " VAULT EXPLORER SUBTITLE GENERATION - RECURSIVE FOLDER RUN" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
foreach ($File in $Files) {
    ...
    if ($SkipExisting -and (Test-Path $SrtPath)) {
        Write-Host "[SKIP] $($File.FullName) -> .srt already exists" -ForegroundColor DarkGray
        $Skipped++
        continue
    }
    ...
    $ArgsList = @("-u", $PythonScript, $File.FullName, $Folder, "--transcribe")
